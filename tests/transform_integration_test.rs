...
use rfb_rs::transform::{Transformer, TransformConfig, Lookups, Company, Partner, CNAE, TaxRegime};
...
fn test_lookups_data_retrieval() {
    ...
    lookups.cnaes.insert(4751201, "Comércio varejista".to_string());
    
    // Test retrieval
    assert_eq!(lookups.get_country(76), Some(&"Brasil".to_string()));
    assert_eq!(lookups.get_city(3550308), Some(&"São Paulo".to_string()));
    assert_eq!(lookups.get_cnae(4751201), Some(&"Comércio varejista".to_string()));
...
fn test_cnae_creation() {
    ...
    assert_eq!(cnae.descricao, "Comércio varejista de produtos de panificação");
}
